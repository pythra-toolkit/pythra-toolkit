...
class Container(Widget):
    ...
    **Key parameters:**
    ...
    def __init__(
        ...
        self.cssClass = (
            cssClass if isinstance(cssClass, list) else [cssClass] if cssClass else []
        )
        ...
        try:
            ...
            if gradient_tuple:
                ...
                if grad_theme.rotationSpeed:
                    ...
                    styles.append(
                        f"background: linear-gradient({grad_theme.gradientDirection}, {gradient_str});"
                    )
                    styles.append("background-size: 400% 400%;")
                    styles.append(
                        f"animation: {keyframes_name} {grad_theme.animationSpeed} {grad_theme.animationTiming} infinite;"
                    )
            ...
            if (
                color and not gradient_tuple
            ):  # Solid color only applies if there's no gradient
            ...
            print(
                f"ERROR generating CSS for Container {css_class} with key {style_key}:"
            )
            traceback.print_exc()
    ...
    **Key parameters:**
    ...
    def __init__(
        ...
        self.onPressed_id = (
            onPressedName
            if onPressedName
            else (onPressed.__name__ if onPressed else None)
        )
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"Warning: Could not unpack style_key for TextButton {css_class}. Using defaults. Key: {style_key}. Error: {unpack_error}"
                )
            ...
            try:
                ...
                ):  # If key stored the object directly
                    style_obj = style_repr
            except Exception as recon_error:
                print(
                    f"Warning: Error reconstructing ButtonStyle for {css_class} from key. Error: {recon_error}"
                )
            ...
            fg_color = getattr(
                style_obj, "foregroundColor", Colors.primary or "#6750A4"
            )
            bg_color = getattr(
                style_obj, "backgroundColor", "transparent"
            )  # Usually transparent
            ...
            shape_obj = getattr(
                style_obj, "shape", BorderRadius.all(20)
            )  # M3 full rounded shape often
            min_height = (
                getattr(style_obj, "minimumSize", (None, 40)) or 40
            )  # M3 min height 40px
            ...
            hover_bg_color = (
                hv_color if hv_color else Colors.rgba(0, 0, 0, 0.08)
            ...
            if isinstance(text_style_obj, TextStyle):
                ...
                text_style_rule = (
                    f".{css_class} > * {{ {base_text_styles} {specific_text_styles} }}"
                )
            else:
            ...
            print(
                f"Error generating CSS for TextButton {css_class} with key {style_key}: {e}"
            )
            traceback.print_exc()
            return f"/* Error generating rule for .{css_class} */"
...
class ElevatedButton(Widget):
    ...
    **Visual hierarchy tip:**
    ...
    def __init__(
        ...
        self.style = (
            style
            or ButtonStyle(  # Provide some sensible defaults for ElevatedButton
        ...
        self.style_key = make_hashable(
            self.style
        )  # Requires ButtonStyle -> hashable tuple/dict

        if self.style_key not in ElevatedButton.shared_styles:
            self.css_class = (
                f"shared-elevatedbutton-{len(ElevatedButton.shared_styles)}"
            )
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"Warning: Could not unpack style_key for ElevatedButton {css_class}. Using defaults. Key: {style_key}. Error: {unpack_error}"
                )
                # Set default values if unpacking fails
                bgColor, fgColor, elevation = (
                    "#6200ee",
                    "white",
                    2.0,
                )  # Basic defaults
            ...
            base_styles_dict = {
                ...
                "padding": (
                    {EdgeInsets(*padding_tuple).to_css_value()}
                    if padding_tuple
                    else "8px 16px"
                ),  # Default padding (M3 like)
                "margin": (
                    {EdgeInsets(*margin_tuple).to_css_value()}
                    if margin_tuple
                    else "4px"
                ),  # Default margin # type: ignore
            ...
            if padding_tuple:
                ...
                try:
                    padding_obj = EdgeInsets(
                        *padding_tuple
                    )  # Assumes tuple is (l,t,r,b)
                    base_styles_dict["padding"] = padding_obj.to_css_value()
                except Exception:
                    pass  # Ignore if padding_tuple isn't valid
            ...
            if margin_tuple:  # type: ignore
                ...
                try:
                    ...
                    pass  # Ignore if margin_tuple isn't valid
            ...
            if side_tuple:
                try:
                    side_obj = BorderSide(
                        *side_tuple
                    )  # Assumes tuple is (w, style, color)
                    shorthand = side_obj.to_css_shorthand_value()
                    if shorthand != "none":
                        base_styles_dict["border"] = shorthand
                    else:
                        base_styles_dict["border"] = "none"
                except Exception:
                    pass
            ...
            if shape_repr:
                if (
                    isinstance(shape_repr, tuple) and len(shape_repr) == 4
                ):  # Assumes (tl, tr, br, bl)
                    try:
            ...
            if textStyle_tuple:
                try:
                    ts_obj = TextStyle(
                        *textStyle_tuple
                    )  # Assumes tuple matches TextStyle init
            ...
            if alignment_tuple:
                try:
                    align_obj = Alignment(
                        *alignment_tuple
                    )  # Assumes tuple is (justify, align)
                    base_styles_dict["display"] = (
                        "inline-flex"  # Use flex to align internal items
                    )
            ...
            if effective_elevation > 0:
                h_offset_y = (
                    1 + (effective_elevation + 2) * 0.5
                )  # Increase elevation effect
            ...
            print(
                f"Error generating CSS for ElevatedButton {css_class} with key {style_key}: {e}"
            )
            traceback.print_exc()
            return f"/* Error generating rule for .{css_class} */"
    ...
    **Accessibility tip:**
    ...
    def __init__(
        ...
        self.current_css_class = (
            f"{self.css_class} {self.cssClass} {disabled_class}".strip()
        )
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            h_padding = (
                padding_obj.to_int_horizontal()
                if isinstance(padding_obj, EdgeInsets)
                else 16
            )
            v_padding = (
                padding_obj.to_int_vertical()
                if isinstance(padding_obj, EdgeInsets)
                else 16
            )
            ...
            if fg_color and fg_color.startswith("#") and len(fg_color) == 7:
                try:
                    ...
                    pass  # Keep defaults if hex parse fails
            ...
            print(
                f"ERROR generating CSS for IconButton {css_class} with key {style_key}:"
            )
            traceback.print_exc()
            return f"/* Error generating rule for .{css_class} */"
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"Warning: Could not unpack style_key for FAB {css_class}. Using defaults. Key: {style_key}. Error: {unpack_error}"
                )
            ...
            base_styles_dict = {
                ...
                "color": fgColor
                or (Colors.onPrimaryContainer or "#21005D"),  # M3 On Primary Container
                "cursor": "pointer",
                "text-decoration": "none",
                "outline": "none",
                "pointer-events": "auto",
            ...
            if style_reconstructed:
                ...
                if bgColor is not None:
                    base_styles_dict["background-color"] = bgColor
                if fgColor is not None:
                    base_styles_dict["color"] = fgColor
            ...
            if eff_elevation >= 1:  # Only show hover elevation if base has elevation
                ...
                if h_elevation >= 12:  # M3 Level 5 Shadow (Max hover approx)
                    h_shadow1 = f"0px 5px 5px -3px rgba(0,0,0,0.2)"
                    h_shadow2 = f"0px 8px 10px 1px rgba(0,0,0,0.14)"
                    h_shadow3 = f"0px 3px 14px 2px rgba(0,0,0,0.12)"
                    hover_shadow_str = (
                        f"box-shadow: {h_shadow1}, {h_shadow2}, {h_shadow3};"
                    )
                elif h_elevation >= 8:  # M3 Level 4 Shadow
                    h_shadow1 = f"0px 3px 5px -1px rgba(0,0,0,0.2)"
                    h_shadow2 = f"0px 7px 10px 1px rgba(0,0,0,0.14)"
                    h_shadow3 = f"0px 2px 16px 1px rgba(0,0,0,0.12)"
                    hover_shadow_str = (
                        f"box-shadow: {h_shadow1}, {h_shadow2}, {h_shadow3};"
                    )
                else:  # Slightly higher than base
                    h_shadow1 = f"0px 3px 5px -1px rgba(0,0,0,0.2)"
                    h_shadow2 = f"0px 6px 10px 0px rgba(0,0,0,0.14)"
                    h_shadow3 = f"0px 1px 18px 0px rgba(0,0,0,0.12)"
            ...
            print(
                f"Error generating CSS for FloatingActionButton {css_class} with key {style_key}: {e}"
            )
            traceback.print_exc()
            return f"/* Error generating rule for .{css_class} */"
    ...
    **Performance tip:**
    ...
    def __init__(
        ...
        if self.style_key not in SingleChildScrollView.shared_styles:
            self.css_class = (
                f"shared-scrollview-{len(SingleChildScrollView.shared_styles)}"
            )
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            print(
                f"ERROR generating CSS for SingleChildScrollView {css_class} with key {style_key}:"
            )
            traceback.print_exc()
            return f"/* Error generating rule for .{css_class} */"
...
class GlobalScrollbarStyle(Widget):
    ...
    **Performance notes:**
    ...
    def __init__(
        self, key: Optional[Key] = None, theme: Optional[ScrollbarTheme] = None
    ):
        ...
        if self.style_key not in GlobalScrollbarStyle.shared_styles:
            ...
            self.css_class = (
                f"global-scrollbar-theme-{len(GlobalScrollbarStyle.shared_styles)}"
            )
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            (
                ...
                track_color,
                radius,
                thumb_padding,  # thumb_padding is now unused but kept for compatibility
            ) = style_key
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            if crossAxisAlignment == CrossAxisAlignment.BASELINE:
                ...
                and mainAxisSize == MainAxisSize.MAX
            ):
            ...
            print(
                f"Error generating CSS for Column {css_class} with key {style_key}: {e}"
            )
            return f"/* Error generating rule for .{css_class} */"
...
class Row(Widget):
    ...
    **Layout tip:**
    ...
    def __init__(
        ...
        self.textDirection = textDirection
        self.verticalDirection = (
            verticalDirection  # Store but may not directly impact Row CSS much
        )
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            (
                ...
                textDirection,
                verticalDirection,
                textBaseline,  # Unpack all stored keys
            ) = style_key
            ...
            if mainAxisSize == MainAxisSize.MAX:
                ...
                width_style = (
                    "width: 100%;"  # Or flex-grow: 1 if inside another flex container
                )
            elif mainAxisSize == MainAxisSize.MIN:
                # Wrap content width
                width_style = (
                    "width: fit-content;"  # Or width: auto; display: inline-flex;
                )
    ...
    **Performance tip:**
    ...
    def __init__(
        ...
        if not isinstance(image, (AssetImage, NetworkImage)):
            raise TypeError(
                "Image widget requires an AssetImage or NetworkImage instance."
            )
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            border_radius_style = (
                f"border-radius: {border_radius.to_css_value()};"
                if border_radius
                else ""
            )
            ...
            print(
                f"Error generating CSS for Image {css_class} with key {style_key}: {e}"
            )
            return f"/* Error generating rule for .{css_class} */"
    ...
    **Performance tip:**
    ...
    def __init__(
        ...
        if not isinstance(icon, IconData):
            raise TypeError(
                "Icon widget requires an IconData object. Use Icons.home, etc."
            )
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            color_rule = (
                f"color: {static_color or 'inherit'};"
...
class _VirtualListViewState(State):
    ...
    **Key Methods and Logic Flow:**
    ...
    def build_item_for_js(self, index: int) -> Dict[str, Any]:
        ...
        html_string = self.framework._generate_html_from_map(
            root_key, result.new_rendered_map
        )
        css_string = self.framework._generate_css_from_details(
            result.active_css_details
        )
        ...
        required_engines = self.framework._analyze_required_js_engines(
            built_tree, result
        )
...
class ListView(Widget):
    ...
    **Always profile and switch to `VirtualListView` if performance suffers.**
    ...
    def __init__(
        ...
        self.cacheExtent = (
            cacheExtent  # Primarily for virtual scrolling logic, not direct CSS
        )
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            if shrinkWrap:
                ...
                if scrollDirection == Axis.VERTICAL:
                    size_style = (
                        "height: fit-content; width: 100%;"  # Fit height, fill width
                    )
                else:  # HORIZONTAL
                    size_style = (
                        "width: fit-content; height: 100%;"  # Fit width, fill height
                    )
            else:
                ...
                size_style = (
                    "flex-grow: 1; flex-basis: 0; width: 100%;"  # Attempt to fill
                )
            ...
            print(
                f"Error generating CSS for ListView {css_class} with key {style_key}: {e}"
            )
            return f"/* Error generating rule for .{css_class} */"
...
class GridView(Widget):
    ...
    **Performance notes:**
    ...
    def __init__(
        ...
        self.mainAxisSpacing = mainAxisSpacing
        self.crossAxisSpacing = crossAxisSpacing
        self.childAspectRatio = max(
            0.01, childAspectRatio
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            if physics == ScrollPhysics.NEVER_SCROLLABLE:
                ...
                physics == ScrollPhysics.ALWAYS_SCROLLABLE
                or physics == ScrollPhysics.BOUNCING
            ):
                overflow_style = f"overflow-{axis_to_scroll}: auto;"
                if physics == ScrollPhysics.BOUNCING:
            ...
            if scrollDirection == Axis.VERTICAL:
                ...
                if childMinWidth:
                    ...
                    grid_template_style = (
                        f"grid-template-columns: repeat({crossAxisCount}, 1fr);"
                    )
                ...
                if childMinWidth:
                    ...
                    grid_template_style = (
                        f"grid-template-rows: repeat({crossAxisCount}, auto);"
                    )
            ...
            child_aspect_ratio_style = (
                f"aspect-ratio: {childAspectRatio};" if childAspectRatio else ""
            )
            # Ensure children handle potential overflow if their content is too big
            child_overflow_style = (
                "overflow: hidden;"  # Simple default, might need configuration
            )
            ...
            child_rule = (
                f".{css_class} > * {{ {child_styles} }}"  # Target direct children
            )
            ...
            print(
                f"Error generating CSS for GridView {css_class} with key {style_key}: {e}"
            )
            return f"/* Error generating rule for .{css_class} */"
...
class Stack(Widget):
    ...
    **Using with `Positioned`:**
    ...
    def __init__(
        ...
        # Store properties
        self.alignment = (
            alignment  # Needs to map to CSS alignment for non-positioned items
        )
        self.textDirection = textDirection
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            if fit == StackFit.expand:
                ...
                size_style = (
                    "width: fit-content; height: fit-content;"  # Size to children
                )
            ...
            alignment_obj = (
                alignment_repr  # Assumes alignment_repr is the Alignment object
            )
            alignment_style = ""
            ...
            print(
                f"Error generating CSS for Stack {css_class} with key {style_key}: {e}"
            )
            return f"/* Error generating rule for .{css_class} */"
...
class Expanded(Widget):
    ...
    **Layout tip:**
    ...
    def __init__(self, child: Widget, key: Optional[Key] = None, flex: int = 1):
...
class AppBar(Widget):
    ...
    def __init__(
        ...
        if bottom:
            children.append(bottom)  # Added bottom as a child
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            leading_styles = (
                "flex-shrink: 0; margin-right: 16px;"  # Prevent shrinking, add margin
            )
            ...
            print(
                f"Error generating CSS for AppBar {css_class} with key {style_key}: {e}"
            )
            return f"/* Error generating rule for .{css_class} */"
...
class BottomNavigationBarItem(Widget):
    ...
    **Important Note on Styling and State:**
    ...
    def __init__(
        ...
        children = [icon]
        if label and (
            (selected and showSelectedLabel) or (not selected and showUnselectedLabel)
        ):
    ...
    def render_props(self) -> Dict[str, Any]:
        ...
        props = {
            ...
            "current_color": (
                self.selectedColor if self.selected else self.unselectedColor
            ),
            "current_font_size": (
                self.selectedFontSize if self.selected else self.unselectedFontSize
            ),
            "show_label": (self.selected and self.showSelectedLabel)
            or (not self.selected and self.showUnselectedLabel),
        }
...
class BottomNavigationBar(Widget):
    ...
    **Key parameters:**
    ...
    def __init__(
        ...
        onTap: Optional[
            Callable[[int], None]
        ] = None,  # Callback function in parent state
        ...
        shadowColor: Optional[str] = Colors.rgba(
            0, 0, 0, 0.15
        ),  # M3 Shadow color (approx)
        ...
        self.item_widgets = []
        actual_onTapName = (
            onTapName if onTapName else (onTap.__name__ if onTap else None)
        )
        if onTap and not actual_onTapName:
            print(
                "Warning: BottomNavigationBar onTap provided without a usable name for JS."
            )

        for i, item in enumerate(items):
            if not isinstance(item, BottomNavigationBarItem):
                print(
                    f"Warning: Item at index {i} is not a BottomNavigationBarItem, skipping."
                )
                continue
        ...
        if self.style_key not in BottomNavigationBar.shared_styles:
            self.css_class = (
                f"shared-bottomnav-{len(BottomNavigationBar.shared_styles)}"
            )
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            print(
                f"Error generating CSS for BottomNavigationBar {css_class} with key {style_key}: {e}"
            )
            return f"/* Error generating rule for .{css_class} */"
...
class Scaffold(Widget):
    ...
    def __init__(
        ...
        persistentFooterButtons: Optional[
            List[Widget]
        ] = None,  # TODO: Handle rendering/layout
        ...
        drawerScrimColor: Optional[str] = Colors.rgba(
            0, 0, 0, 0.4
        ),  # M3 Scrim color (approx)
        ...
        super().__init__(
            key=key, children=layout_children
        )  # Pass key, body is main child
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            ...
            print(
                f"Error generating CSS for Scaffold {css_class} with key {style_key}: {e}"
            )
            return f"/* Error generating rule for .{css_class} */"
    ...
    def generate_css_rule(style_key: Tuple, css_class: str) -> str:
        ...
        try:
            decoration = InputDecoration(
                ...
                label=style_key[0],
                hintText=style_key[1],
                errorText=style_key[2],
                fillColor=style_key[3],
            ...
            print(
                f"Error unpacking style_key for TextField {css_class}. Using default decoration. Error: {e}"
            )
            decoration = InputDecoration()
